...
public class UINavigator : IUINavigator
{
    ...
    private void InitNavigationsSystems()
    {
        ...
        _popupNavigationSystem = new PopupNavigationSystem();
    ...
    public IUINavigator WithData<T>(T data) where T : BaseVm
    {
    ...
    {
        if (modalType == UIModalType.All || modalType == UIModalType.Screens)
        {
            _screenNavigationSystem.HideAllViews();
            _currentScreen = null;
        }

        if (modalType == UIModalType.All || modalType == UIModalType.Popups)
        {
            _popupNavigationSystem.CloseAll();
            _currentPopup = null;
        }

        if (modalType == UIModalType.All || modalType == UIModalType.BottomSheets)
        {
